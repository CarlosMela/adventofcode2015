console.log(getSolution(getTransformer1));
console.log(getSolution(getTransformer2));

function getSolution(transformerFn){
  var initialState = getGrid(1000,0);
  for(var op of getInput()){
    var x1y1x2y2 = getPoints(op);
    var transformer = transformerFn(op);
    initialState = runThrow(initialState,parseInt(x1y1x2y2[0]),parseInt(x1y1x2y2[1]),
    parseInt(x1y1x2y2[2]),parseInt(x1y1x2y2[3]),transformer);
  }
  return count(initialState);
}

function getPoints(operation){
  return operation.match(/\d+/g);
}

function getTransformer1(operation){
  if(/turn on/.test(operation)){
    return function(){return 1;};
  }else if (/turn off/.test(operation)) {
...
function getTransformer2(operation){
  if(/turn on/.test(operation)){
    return function(x){return x+1;};
  }else if (/turn off/.test(operation)) {
    return function(x){return x>0?x-1:0;};
  }else {
...
function runThrow(grid,x1,y1,x2,y2,transformer){
  for(var i=x1;i<=x2;i++){
    for(var j =y1;j<=y2;j++){
      grid[i][j] = transformer(grid[i][j]);
    }
  }
  return grid;
}
