...
function solution1(){
  processOperation(getInput1(),{});
}

function solution2(){
  processOperation(getInput2(),{});
}

function processOperation(ops,wires){
  if(!ops.length){
    console.log("SOLUTION:"+ wires.a);
  }else {
    const op = ops.shift();
    let newWires = evalOp(op,wires);
    //timeouts to avoid Maximum call stack size exceeded
    if(newWires){
      setTimeout(()=>processOperation(ops,newWires),10);
    }else{
      ops.push(op);
      setTimeout(()=>processOperation(ops,wires),10);
    }
  }
}

function evalOp(op,wires){
  const parts = op.split(" -> ");
  const provider = parts[0];
  const variable = parts[1];
  const value = providerToValue(provider,wires);
  if(value!==null){
    const intval = parseInt(value);
    let command = `wires["${variable}"] = ${value}`;
    eval(command);
    return wires;
  }
  return false;
